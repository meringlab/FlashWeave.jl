    ...
    @testset "$test_name" begin
        for cond_mode in ["uni", "condZ1", "condZ3"]
            @testset "$cond_mode" begin
                ...
                if cond_mode == "uni"
                    @test all([compare_test_results(r1, r2) for (r1, r2) in zip(FlashWeave.Tests.test(1, collect(2:50), data_norm, test_name), exp_res)])
                elseif cond_mode == "condZ1"
                    test_res = FlashWeave.Tests.test(31, 21, [7], data_norm, test_name)
                    @test compare_test_results(test_res, exp_res)
                elseif cond_mode == "condZ3"
                    test_res = FlashWeave.Tests.test(31, 21, [7, 14, 18], data_norm, test_name)
                    @test compare_test_results(test_res, exp_res)
                end
