...
function prepare_univar_results(data, test_name, alpha, hps, n_obs_min, FDR, levels,
...
function make_preclustering(precluster_sim, data, target_vars, cor_mat, levels, test_name, all_univar_nbrs, cluster_mode,
    verbose)
...
function infer_cond_neighbors(target_vars, data, all_univar_nbrs, parallel, nonsparse_cond, recursive_pcor,
    ...
    if recursive_pcor && is_zero_adjusted(hiton_kwargs[:test_name])
        hiton_kwargs[:cor_mat] = zeros(cont_type, size(data, 2), size(data, 2))
    end
    ...
    if parallel == "single" || nprocs() == 1
        nbr_results = [si_HITON_PC(x, data; univar_nbrs=all_univar_nbrs[x], hiton_kwargs...) for x in target_vars]
    else
        # embarassing parallelism
        if parallel == "multi_ep"
            nbr_results = @parallel (vcat) for x in target_vars
                si_HITON_PC(x, data; univar_nbrs=all_univar_nbrs[x], hiton_kwargs...)
            end

        # interleaved mode
        elseif endswith(parallel, "il")
            il_dict = interleaved_backend(target_vars, data, all_univar_nbrs, hiton_kwargs; parallel=parallel,
    ...
    Dict{Int,HitonState}(zip(target_vars, nbr_results))
end


function learn_graph_structure(target_vars, data, all_univar_nbrs, parallel, recursive_pcor,
    cont_type, time_limit, nonsparse_cond, verbose, track_rejections, hiton_kwargs, interleaved_kwargs)
    ...
    # else, run full conditioning
        ...
        nbr_results_dict = infer_cond_neighbors(target_vars, data, all_univar_nbrs, parallel, nonsparse_cond, recursive_pcor,
                                        cont_type, verbose, hiton_kwargs, interleaved_kwargs)

        nbr_dict = Dict{Int,OrderedDict{Int,Tuple{Float64,Float64}}}([(target_var, nbr_state.state_results) for (target_var, nbr_state) in nbr_results_dict])
...
function map_clusters_to_variables(nbr_dict, all_univar_nbrs, rej_dict, clust_var_dict, track_rejections)
...
function LGL(data::AbstractMatrix{ElType}; test_name::String="mi", max_k::Integer=3, alpha::AbstractFloat=0.01,
    ...
    convergence_threshold::AbstractFloat=0.01, FDR::Bool=true, parallel::String="single",
    ...
    track_rejections::Bool=false, cluster_mode="greedy") where {ElType<:Real}
    """
    ...
    hiton_kwargs = merge!(hiton_kwargs, Dict(:levels => levels, :cor_mat => cor_mat))
    ...
    nbr_dict, unfinished_state_dict, rej_dict = learn_graph_structure(target_vars, data, all_univar_nbrs, parallel, recursive_pcor,
                                                                      cont_type, time_limit, nonsparse_cond,
                                                                      verbose, track_rejections, hiton_kwargs, interleaved_kwargs)
